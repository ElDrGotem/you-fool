...
async client.on('message', msg => {
	...
	async function sendQueue(msg){
		...
		msg.channel.send({embed:{title:`Now playing: ${curQ[0].name}`,
				description:out,
				color:config.color,
				footer:{text:"Requested by "+msg.author.tag,
					icon_url:msg.author.avatarURL}}});*/
	...
	/*function MCStatus(msg) {
		var serverURL = msg.content.slice(10,msg.content.length);
		$.getJSON('https://api.mcsrvstat.us/2/'+serverURL,function(status){
